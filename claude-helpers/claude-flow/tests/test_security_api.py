...
class TestStartSecurityCheck:
    ...
    def test_start_security_check_returns_started_status(self, client):
        """Test that starting a security check returns correct response structure."""
        response = client.post("/api/security/check")
...
class TestListSecurityReports:
    ...
    def test_list_reports_empty_when_no_reports_exist(self, client):
    ...
    def test_list_reports_returns_existing_reports(
        self, client, sample_security_report  # noqa: ARG002
    ):
...
class TestGetSecurityReport:
    ...
    def test_get_report_returns_content(
        ...
        # sample_security_report fixture creates the file as a side effect
        response = client.get("/api/security/report/security-analysis-2026-01-09.md")
        assert response.status_code == 200
    ...
    def test_get_report_404_for_nonexistent_file(self, client):
        """Test that GET /api/security/report/{filename} returns 404 for missing files."""
        response = client.get("/api/security/report/security-analysis-2099-01-01.md")
        assert response.status_code == 404
        assert "not found" in response.json()["detail"]

    def test_get_report_prevents_path_traversal_with_slash(self, client):
        ...
        response = client.get("/api/security/report/../../../etc/passwd")
        assert response.status_code in [400, 404]

    def test_get_report_prevents_path_traversal_with_backslash(self, client):
        """Test that path traversal with backslash is blocked."""
        response = client.get("/api/security/report/..\\..\\..\\etc\\passwd")
        assert response.status_code == 400
        assert "Invalid filename" in response.json()["detail"]

    def test_get_report_prevents_path_traversal_with_dotdot(self, client):
        """Test that path traversal with .. is blocked."""
        response = client.get("/api/security/report/security-analysis-..md")
        assert response.status_code == 400
        assert "Invalid filename" in response.json()["detail"]

    def test_get_report_rejects_invalid_prefix(self, client):
        """Test that files not starting with security-analysis- are rejected."""
        response = client.get("/api/security/report/random-file.md")
        assert response.status_code == 400
        assert "Invalid filename format" in response.json()["detail"]
        assert "security-analysis-" in response.json()["detail"]

    def test_get_report_rejects_invalid_suffix(self, client):
        """Test that files not ending with .md are rejected."""
        response = client.get("/api/security/report/security-analysis-2026-01-09.txt")
        assert response.status_code == 400
        assert "Invalid filename format" in response.json()["detail"]

    def test_get_report_rejects_empty_filename(self, client):
...
class TestSecurityValidation:
    ...
    def test_traversal_patterns_blocked(self, client):
        ...
        for filename in malicious_filenames:
            response = client.get(f"/api/security/report/{filename}")
            # Should be blocked with 400 or 404, never 200
            assert response.status_code != 200, f"Failed for: {filename}"

    def test_only_security_analysis_files_allowed(self, client):
